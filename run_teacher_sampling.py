...
def main(_):
    ...
    if FLAGS.balanced_sampling:
        print(train_dataset.sample(1))
        # Create a separate replay buffer so that we can sample from both the training dataset and the replay buffer.
        # example_transition = jax.tree_util.tree_map(lambda x: x[0], train_dataset.sample(1))
        # replay_buffer = ReplayBuffer.create(example_transition, size=FLAGS.buffer_size)
        # example_transition = jax.tree_util.tree_map(
        #     lambda x: x.astype(np.float32) if (isinstance(x, np.ndarray) and x.dtype == np.float64) else x,
        #     example_transition,
        # )
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
        if i <= FLAGS.offline_steps:
            ...
            noises = jax.random.normal(key, (1, action_dim))
            action = agent.compute_flow_actions(observations=ob[None, ...], noises=noises)
