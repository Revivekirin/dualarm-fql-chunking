...
class ReplayBuffer(Dataset):
    def __init__(
        ...
        dataset_dict = dict(
            ...
            # infos=np.empty((capacity,), dtype=object),
        )
    ...
    def get_iterator(self, queue_size: int = 2, sample_args: dict = {}, device=None):
        ...
        def enqueue(n):
            for _ in range(n):
                ...
                # data = _device_put_except_infos(data, device=device)
                # queue.append(data)
...
def print_frozen_shapes(data: frozen_dict.FrozenDict, prefix=""):
    for k, v in data.items():
        if isinstance(v, frozen_dict.FrozenDict):
            # nested FrozenDict 재귀 탐색
            print_frozen_shapes(v, prefix + k + ".")
        else:
            try:
