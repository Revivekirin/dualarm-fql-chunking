...
import numpy as np
from data.dataset import DatasetDict, _sample
    ...
    def __init__(
        ...
        for pixel_key in self.pixel_keys:
            ...
            if len(shape) == 3:
                ...
                self._num_stack = shape[0]

            else:
                raise ValueError(f"Unexpected image shape: {shape}")
            print(f"[MemoryEfficientReplayBuffer] _num_stack: {self._num_stack}")
            print(f"[MemoryEfficientReplayBuffer] pixel_obs_space: {pixel_obs_space.shape[0]}")
            ...
            print(f"[MemoryEfficientReplayBuffer] observation_space ", observation_space)
